...
class addFoodWithBarcode extends StatefulWidget {
  ...
  const addFoodWithBarcode({Key? key, required this.scanedBarcode})
      : super(key: key);
...
class _addFoodWithBarcodeState extends State<addFoodWithBarcode> {
    ...
    try {
      for (String barcode in widget.scanedBarcode) {
        ...
        var response = await Dio()
            .get(
                'https://openapi.foodsafetykorea.go.kr/api/$scanBarcodeKey/C005/json/1/5/BAR_CD=$barcode')
            .then((value) {
  ...
  /*Food makeFoodFromBarcode({required String data}){
    ...
    Food result = new Food(
        ...
        index: 0,
        status: false,
        name: parsedData['C005']['row'][0]['PRDLST_NM'],
        num: 1,
        category: '-1',
        method: 0,
        displayType: true,
